...
var ColorParser = {
    parse: function(color) {
        ...
        return this._parseKnownColor(color) ?? this._parseHexColor(color) ?? this._parseRgbColor(color);
    },
    _parseKnownColor(color) {
        for (const knownColor of knownColors) {
            if (knownColor.name === color) {
    ...
    _parseHexColor(color) {
        const hexColorRegExp = /^#?((?:[\da-f]{3}){1,2}|(?:[\da-f]{4}){1,2})$/i;
        const matches = hexColorRegExp.exec(color);
        if (!matches) {
            return null;
        }

        const hexColor = matches[1];
        let rgba;
    ...
    _parseRgbColor(color) {
        const rgbColorRegExp = /^rgba?\((\d{1,3})(?:,\s*|\s+)(\d{1,3})(?:,\s*|\s+)(\d{1,3})(?:(?:,\s*|\s+)(\d{1,3}))?\)$/i;
        const matches = rgbColorRegExp.exec(color);
        if (!matches) {
            return null;
        }
